    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 selection:bg-pink-100 selection:text-pink-600">
      ...
      <section className="py-16 px-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { title: "Exclusive Dining Deals", desc: "Unlock special discounts and set menus to savor the authentic flavors of Asia." },
            { title: "Custom Tour Packages", desc: "Bespoke travel itineraries that cater to a wide range of interests." },
            { title: "Hotel Booking", desc: "Access a vast network of accommodations with competitive rates and real-time availability." },
            { title: "Activities", desc: "Discover and book exciting local tours and unique activities to enrich your travel experience." }
          ].map((item, idx) => (
            ...
            <ul className="space-y-4">
              {[
                ...
                <li key={idx} className="flex flex-col border-l-2 border-pink-200 pl-4 hover:border-pink-500 transition-colors">
                  ...
                  <p className="text-slate-600 text-[11px] leading-snug">{point.val}</p>
                </li>
              ))}
            </ul>
          </div>
      ...
      <footer className="bg-slate-950 py-10 px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10">
          <div className="text-center md:text-left">
            ...
            <h3 className="text-2xl font-black text-white uppercase tracking-tighter">Plan your next adventure</h3>
          </div>
          ...
          <Link 
            ...
            target="_blank" 
            className="px-8 py-3 bg-white text-black font-black uppercase tracking-widest text-[10px] rounded-full hover:bg-pink-500 transition-all shadow-lg"
          >
            Visit Website
          </Link>
        </div>
